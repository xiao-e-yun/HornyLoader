...
class ExecuteAuxClassOperator(bpy.types.Operator):
    ...
    def exportframe(self,context,filename):
        ...
        try:
            ...
            for obj in mainobjects.copy():
                print("Searching partner for "+obj.name)
                collision = False
                for col in collectionstojoin:
                    if not collision and obj.name.lower().startswith(col.name.lower()):
                        ...
                        break
                if not collision: self.report({'WARNING'},"Collection not found for object: " + obj.name)
...
class ExportAnimationOperator(ExecuteAuxClassOperator):
    ...
    def execute(self,context):
        ...
        target_folder = output_folder + ".variants"

        # if os.path.exists(new):
        #     shutil.rmtree(new)
        if os.path.exists(target_folder): raise Fatal("Remove 'output.variants' to use generate variants")
        ...
        subprocess.run([YOUR_7Z_PATH,"a",os.path.join(target_folder,"package.7z"),os.path.join(target_folder,"*"),"-sdel"]) 
        #
        with open(target_folder+"/config.hl.json", 'w') as f:
