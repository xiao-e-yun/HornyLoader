...
def export_3dmigoto_genshin(operator, context, object_name, vb_path, ib_path, fmt_path, use_foldername, ignore_hidden, only_selected, no_ramps, delete_intermediate, credit, Outline_Properties):
    ...
    #   the folder name
    folder = os.path.join(os.path.dirname(vb_path),"assets")
    assert_list = os.listdir(folder)
    
    if use_foldername or (not [obj for obj in scene.objects if object_name.lower() in obj.name.lower()] \
            or not [file for file in assert_list if object_name.lower() in file.lower()]):
        object_name = scene.world.name
        print(f"loading {object_name} (from `Scene Name`)")
        if object_name == "World": raise Fatal("ERROR: Change name of Scene to Char Name")
    ...
    if "hash.json" in assert_list:
        print("Hash data found in character folder")
        with open(os.path.join(folder, "hash.json"), "r") as f:
    ...
    for k in range(len(all_base_classifications)):
        ...
        for i, obj in enumerate(relevant_objects):
            ...
            vb_path  = os.path.join(folder, current_name + classification + ".vb")
            ib_path  = os.path.join(folder, current_name + classification + ".ib")
            fmt_path = os.path.join(folder, current_name + classification + ".fmt")
            ...
            if len(mesh.polygons) == 0:
                ...
                # Write format reference file
                write_fmt_file(open(fmt_path, 'w'), vb, ib)
    # generate_mod_folder(folder, object_name, no_ramps, credit)
...
def generate_mod_folder(path: str, character_name: str, no_ramps: bool, variants: str):
            ...
            for i in range(len(component["object_indexes"])):
                ...
                if component["blend_vb"]:
                    ...
                    try:
                        ...
                        raise "ERROR: Unable to find {current_name} {current_object} when exporting. Double check the object exists and is named correctly"
                    position += x
                    blend += y
                    texcoord += z
                    position_stride = 40
    ...
    other_res = ""
    if variants:
        constant_ini += f"[Constants]\nglobal $active = 0\nglobal $variantsinfo = 0\n\n[Present]\npost $active = 0\nrun = CommandListVariantsInfo\n\n"
        command_ini += "[CommandListVariantsInfo]\nif $variantsinfo == 0 && $active == 1\n" \
                       "\tpre Resource\\ShaderFixes\\help.ini\\Notification = ResourceVariantsInfo\n" \
                       "\tpre run = CustomShader\\ShaderFixes\\help.ini\\FormatText\n" \
                       "\tpre $\\ShaderFixes\\help.ini\\notification_timeout = time + 12.0\n" \
                       "\t$variantsinfo = 1\n" \
                       "endif\n\n"
        other_res += f'[ResourceVariantsInfo]\ntype = Buffer\ndata = "{variants} (by xiaoeyun)"\n\n'
...
def load_hashes(path: str, name: str, hashfile: str):
    parent_folder = os.path.join(path, "../")
    if hashfile not in os.listdir(path):
        ...
        if "hash_info.json" not in os.listdir(parent_folder):
            raise "Cannot find hash information, check hash.json in folder"
...
def create_mod_folder(parent_folder:str, name:str):
...
def collect_vb(folder:str, name:str, classification: str, stride: int): 
    position = bytearray()
    blend = bytearray()
    texcoord = bytearray()
...
def collect_ib(folder: str, name: str, classification: str, offset: int):
    ib = bytearray()
...
def collect_vb_single(folder: str, name: str, classification: str, stride: int): 
    result = bytearray()
    with open(os.path.join(folder, f"{name}{classification}.vb"), "rb") as f:
        data = f.read()
        data = bytearray(data)
